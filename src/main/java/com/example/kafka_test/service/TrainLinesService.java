...
public class TrainLinesService {
    ...
    public double queryDistanceById(String id1,String id2){
        ...
        if(distanceDoubleHashMap.containsKey(d1))
            return distanceDoubleHashMap.get(d1);
            //考虑反方向列车的情况，距离只需要存一份
        else if(distanceDoubleHashMap.containsKey(d2))
            return distanceDoubleHashMap.get(d2);
        else return -1;
    }
    ...
    public double calculateDirection(String source,String target) {
        ...
        if(targetCoordinate[1]>=sourceCoordinate[1])
            return theta;
        else
            return 360.0 + theta;
    }
